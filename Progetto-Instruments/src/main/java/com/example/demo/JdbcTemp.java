...
public class JdbcTemp {
     ...
     */
    public int insertstrumento(String nome, String marca, double prezzo, String url) {
        String query = "INSERT INTO strumenti(nome, marca, prezzo, url, pezzi, pezziV) VALUES (?, ?, ?, ?,?,?)";
        return jdbcTemplateObject.update(query, nome, marca,  prezzo, url, 50, 50);
    ...
    public void aggiungipezzi(String nome, int quantita) {
        String sql = "UPDATE strumenti SET pezzi = pezzi + ? WHERE nome = ?";
        jdbcTemplateObject.update(sql, quantita, nome);
    }
    ...
    private static void update(String sql, int quantita, String nome) {
    ...
    public ArrayList<strumento> getLista() {
        String query = "SELECT * FROM strumenti";

        return jdbcTemplateObject.query(query, new ResultSetExtractor<ArrayList<strumento>>() {
        /*
         ...
         * */
        	@Override
            public ArrayList<strumento> extractData(ResultSet rs) throws SQLException, DataAccessException {
                ArrayList<strumento> listaP = new ArrayList<>();

                // Itera sui risultati della query e crea un nuovo oggetto pc per ciascun record.
                while (rs.next()) {
                    strumento p1 = new strumento();
